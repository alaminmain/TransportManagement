...
{
    public partial class SPRApproved2 : System.Web.UI.Page

    {
        ...
        private DataTable LoadBasicGrid(string searchKey)
        {
            try
            {
                ...
                string sql = @"SELECT     PurRequisition.ComCode, PurRequisition.ReqNo, PurRequisition.CateCode, PurRequisition.ProdSubCatCode, PurRequisition.ReqDate, 
                      ...
                      ItemSubCategory ON PurRequisition.CateCode = ItemSubCategory.CateCode AND PurRequisition.ProdSubCatCode = ItemSubCategory.ProdSubCatCode and(Status='0' or Status='1') 
...
.Where(r => r.Field<String>("ReqNo").ToUpper().Contains(searchKey.ToUpper())
       || r.Field<String>("Remark").ToUpper().Contains(searchKey.ToUpper()));
        ...
        {
            string sql1 = @"select IT.CateCode,IT.ProdSubCatCode,IT.ProductCode,IT.ProductName,IT.ProductBName,ITS.ReqQTY,
                           IT.Reorder,IT.ProductDiscription,IT.DealPrice,IT.Discontinued,IT.DistPrice,IT.OperPrice,IT.UnitType,
                           ITS.StoreCode,ITS.PhyStock from Item IT inner join ItemStock ITS on IT.ProductCode=ITS.ProductCode and IT.StoreCode=ITS.StoreCode
                           where (ITS.PhyStock+ITS.ReqQTY)<IT.Reorder and it.ProdSubCatCode=@ProdSubCatCode and it.CateCode=@CateCode";
        ...
        private void loadSPRForm()
        {
            try
            {
                ...
                string sql = @"SELECT     PR.ComCode, PR.ReqNo, CONVERT(VARCHAR(10), PR.ReqDate, 103) AS ReqDate, PR.Remark, PR.CateCode, IT.ProdSubCatCode, PRD.ReqSLNo, PRD.ProductCode, 
...
FROM         Item AS IT INNER JOIN       PurRequisition AS PR INNER JOIN
                ...
                if (dt.Rows.Count > 0)
                {
                    ...
                    //if (ImergencyREQ == 1)
                    //{
                    //    ckEmergencyReq.Checked = true;
                    //}
                    //else
                    //{
                    //    ckEmergencyReq.Checked = false;
                    //}
