...
{
    public partial class PullReceive : System.Web.UI.Page
    {
        ...
        private DataTable GetVehicleMovement(string searchKey)
        {
            try
            {
                ...
                using (VehicleInfoGateway gatwayObj = new VehicleInfoGateway())
                {
                    ...
                    foundRows = dt2.Select("[VehicleStatus]='0'");
    ...
    .Where(r => r.Field<String>("VehicleID").Contains(searchKey) || r.Field<String>("VehicleNo").ToUpper().Contains(searchKey.ToUpper()));
        ...
        {
            if (String.IsNullOrEmpty(lblDriverCode.Text)||String.IsNullOrEmpty(hfShowTrip.Value))
                return false;
        ...
        protected void btnSave_Click(object sender, EventArgs e)
        {
            if (isMandatoryFieldValidate())
            {
                ...
                using (VehicleInfoGateway gatwayObj = new VehicleInfoGateway())
                {
                    string vno = gatwayObj.InsertUpdateVehicleMovement(1, moveRegNo, movDate, vehicleId, DriverCode,
                        Remarks, vehicleStatus, VStatus, userCode);


                    ScriptManager.RegisterClientScriptBlock(this, this.GetType(), "Alert", "alert('Record Saved');",
                        true);
