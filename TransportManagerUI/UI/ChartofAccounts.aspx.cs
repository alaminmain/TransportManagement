...
namespace TransportManagerUI.UI
{
    public partial class ChartofAccounts : System.Web.UI.Page
    {
        ...
        {
            if (String.IsNullOrEmpty(txtAccountDesc.Text))
                return false;
            else
                return true;
        }
    ...
    .Where(r => r.Field<String>("AccountCode").ToUpper().Contains(searchKey.ToUpper())
        ...
        protected void btnSave_Click(object sender, EventArgs e)
        {
            if (isMandatoryFieldValidate())
            {
                ...
                string accCategory5 = ddlAccountCategory.SelectedValue;
                string userId = Session["UserName"].ToString();

                using (ChartOfAccountGateway gatwayObj = new ChartOfAccountGateway())
                {
                    string accountCode = gatwayObj.InsertUpdateChartOfAccount(comCode1, accountCode2, accountDesc3, remarks, accCategory5, userId);
        ...
        protected void gvlistofBasicData_SelectedIndexChanged(object sender, EventArgs e)
        {
            try
            {
                ...
                ddlAccountCategory.SelectedValue = dt.Rows[0]["AccCategory"].ToString();
